...
export const RunestonePopup: React.FC<RunestonePopupProps> = ({ runestone }) => {
  return (
    <div className="p-4 max-w-sm">
      <h3 className="font-bold text-xl mb-3 text-primary border-b border-gray-200 pb-2">
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="text-sm text-gray-600">{runestone.found_location}</p>
          <p className="text-xs text-gray-500">{runestone.parish}</p>
        </div>
        
        <div>
          <h4 className="font-semibold text-sm text-gray-700 mb-1">Details</h4>
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div><span className="font-medium">Material:</span> {runestone.material || 'Unknown'}</div>
            <div><span className="font-medium">Dating:</span> {runestone.dating || 'Unknown'}</div>
            <div><span className="font-medium">Type:</span> {runestone.rune_type || 'Unknown'}</div>
            <div><span className="font-medium">Style:</span> {runestone.material_type || 'Unknown'}</div>
          </div>
        </div>
        
        {runestone.english_translation && (
          <div>
            <h4 className="font-semibold text-sm text-gray-700 mb-1">English Translation</h4>
            <p className="text-sm text-gray-600 leading-relaxed">{runestone.english_translation}</p>
          </div>
        )}
        
        {runestone.swedish_translation && (
          <div>
            <h4 className="font-semibold text-sm text-gray-700 mb-1">Swedish Translation</h4>
            <p className="text-sm text-gray-600 leading-relaxed">{runestone.swedish_translation}</p>
          </div>
        )}
        
        {runestone.norse_text && (
          <div>
            <h4 className="font-semibold text-sm text-gray-700 mb-1">Norse Text</h4>
            <p className="text-sm text-gray-600 italic leading-relaxed">{runestone.norse_text}</p>
          </div>
        )}
      </div>
    </div>
